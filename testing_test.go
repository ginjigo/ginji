...
func TestPerformRequest(t *testing.T) {
	...
	app.Get("/test", func(c *Context) error {
		return c.Text(StatusOK, "Hello Test")
	})
...
func TestPerformJSONRequest(t *testing.T) {
	app := New()
	app.Post("/api/user", func(c *Context) error {
		var data map[string]string
		if err := c.BindJSON(&data); err != nil {
			return c.JSON(StatusBadRequest, H{"error": err.Error()})
		}
		return c.JSON(StatusOK, H{"received": data["name"]})
	})
...
func TestPerformFormRequest(t *testing.T) {
	...
	app.Post("/submit", func(c *Context) error {
		...
		return c.JSON(StatusOK, H{"name": name})
	})
...
func TestRequestBuilder(t *testing.T) {
	...
	app.Get("/hello", func(c *Context) error {
		...
		return c.JSON(StatusOK, H{"auth": auth})
	})
...
func TestRequestBuilderCookie(t *testing.T) {
	app := New()
	app.Get("/cookie-test", func(c *Context) error {
		cookie, err := c.Cookie("session")
		if err != nil {
			return c.Text(StatusBadRequest, "no cookie")
		}
		return c.Text(StatusOK, cookie.Value)
	})
...
func TestResponseHelpers(t *testing.T) {
	...
	app.Get("/json", func(c *Context) error {
		return c.JSON(StatusOK, H{"message": "hello"})
	})
...
func TestAssertHelpers(t *testing.T) {
	app := New()
	app.Get("/status-test", func(c *Context) error {
		return c.JSON(StatusCreated, H{"status": "created"})
	})
...
func TestPerformMultipartRequest(t *testing.T) {
	...
	app.Post("/upload", func(c *Context) error {
		...
		if err != nil {
			return c.Text(StatusBadRequest, "no file")
		}
		return c.JSON(StatusOK, H{"name": name, "filename": file.Filename})
	})
