...
func (r *Req) Query(key string) string {
	return r.URL.Query().Get(key)
}

// QueryDefault returns the query parameter value or default if not present.
func (r *Req) QueryDefault(key, defaultValue string) string {
...
func (c *Context) DeepCopy() *Context {
	...
	// Copy handlers slice
	cp.handlers = make([]Handler, len(c.handlers))
	copy(cp.handlers, c.handlers)

	// Service scope is shared (read-only operations in handlers are safe)
	// If handlers need to modify services, they should do so through the container
	cp.services = c.services
...
// TextOK writes a string to the response with 200 OK status.
// Convenience method for common success text responses.
func (c *Context) TextOK(text string) error {
	return c.Text(http.StatusOK, text)
}

// HTMLOK writes an HTML string to the response with 200 OK status.
// Convenience method for common success HTML responses.
func (c *Context) HTMLOK(html string) error {
	return c.HTML(http.StatusOK, html)
}

// Var stores a value in the context (alias for Set for cleaner API).
// Inspired by Hono.js variable storage pattern.
func (c *Context) Var(key string, value any) {
	c.Set(key, value)
}

// GetVar retrieves a value from the context (alias for Get).
func (c *Context) GetVar(key string) (any, bool) {
	return c.Get(key)
}

// Fail sends an error response with the given status code and message.
// Convenience method for quick error responses.
...
func (c *Context) FailWithData(code int, message string, data any) error {
	return c.JSON(code, H{
		"error":   true,
		"message": message,
		"data":    data,
...
// It now returns error to allow middleware to handle errors properly.
