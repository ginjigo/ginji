...
func TestConditionalMiddleware(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			app.Get(tt.path, func(c *Context) error {
...
func TestCombineMiddleware(t *testing.T) {
	...
	mw1 := func(c *Context) error {
		execution += "1-"
		return c.Next()
	}

	mw2 := func(c *Context) error {
		execution += "2-"
		return c.Next()
	}
	...
	c.handlers = []Handler{Handler(combined), handler}
	_ = c.Next()
...
func TestSkipAndOnly(t *testing.T) {
	...
	c.handlers = []Handler{Handler(skipped), func(c *Context) error { return nil }}
	_ = c.Next()
	...
	c = NewContext(w, req, nil)
	c.handlers = []Handler{Handler(only), func(c *Context) error { return nil }}
	_ = c.Next()
...
func TestUnlessCondition(t *testing.T) {
	...
	c.handlers = []Handler{Handler(unless), func(c *Context) error { return nil }}
	_ = c.Next()
