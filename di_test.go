...
func TestConstructorInjection(t *testing.T) {
	...
	container.RegisterSingleton("ginji.ILogger", func() ILogger {
		return &simpleLogger{messages: make([]string, 0)}
	})

	// Register user service with dependency injection
	container.RegisterTransient("*ginji.UserService", func(logger ILogger) *UserService {
...
func TestGetServiceGeneric(t *testing.T) {
	...
	// Register a service
	container.RegisterSingleton("logger", func() ILogger {
		return &simpleLogger{messages: make([]string, 0)}
	})
...
func TestEngineServiceIntegration(t *testing.T) {
	...
	app.RegisterSingleton("logger", func() ILogger {
		return &simpleLogger{messages: make([]string, 0)}
	})

	app.RegisterScoped("*ginji.UserService", func(logger ILogger) *UserService {
		return NewUserService(logger)
	})
