...
func TestRouterComplexPatterns(t *testing.T) {
	...
	app.Get("/users/:id", func(c *Context) error {
		handler1Called = true
		return c.JSON(StatusOK, H{"handler": "1", "id": c.Param("id")})
	})

	app.Get("/users/new", func(c *Context) error {
		handler2Called = true
		return c.JSON(StatusOK, H{"handler": "2"})
	})

	app.Get("/users/:id/profile", func(c *Context) error {
		handler3Called = true
		return c.JSON(StatusOK, H{"handler": "3", "id": c.Param("id")})
	})
...
func TestRouterWildcardRoutes(t *testing.T) {
	...
	app.Get("/files/*filepath", func(c *Context) error {
		...
		return c.JSON(StatusOK, H{"filepath": filepath})
	})
...
func TestRouterMethodNotAllowed(t *testing.T) {
	...
	app.Get("/users", func(c *Context) error {
		return c.Text(StatusOK, "GET users")
	})

	app.Post("/users", func(c *Context) error {
		return c.Text(StatusOK, "POST users")
	})
...
func TestRouterGroupsWithMiddleware(t *testing.T) {
	...
	globalMW := func(c *Context) error {
		globalMWCalled = true
		return c.Next()
	}

	groupMW := func(c *Context) error {
		groupMWCalled = true
		return c.Next()
	}
...
func TestRouterConflictingRoutes(t *testing.T) {
	...
	app.Get("/posts/:id", func(c *Context) error {
		return c.JSON(StatusOK, H{"route": "posts-id", "id": c.Param("id")})
	})

	app.Get("/posts/latest", func(c *Context) error {
		return c.JSON(StatusOK, H{"route": "posts-latest"})
	})
...
func TestRouterCaseSensitivity(t *testing.T) {
	...
	app.Get("/Users", func(c *Context) error {
		return c.Text(StatusOK, "uppercase")
	})

	app.Get("/users", func(c *Context) error {
		return c.Text(StatusOK, "lowercase")
	})
